...
public class TestHelper {
    ...
    public static Campaign makeCampaign(int campaignId) {
        ...
        return new Campaign(campaignId, String.format("Campaign #%s", campaignId), String.format("Description #%s", campaignId), startDate, endDate, new BigDecimal("30000.00"));
    }

    public static Backer makeBacker(int backerId) {
        return new Backer(backerId, String.format("Backer #%s", backerId), String.format("backer%s@example.com", backerId));
    }
    ...
    public static <T> Result<T> makePayloadResult(T payload) {
    ...
    public static <T> Result<T> makeNotFoundResult() {
        Result<T> result = new Result<>();
        result.setNotFound();
        return result;
    }
