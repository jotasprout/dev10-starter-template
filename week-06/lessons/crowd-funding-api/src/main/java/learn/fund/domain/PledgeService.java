...
public class PledgeService {
    ...
    public PledgeService(BackerRepository backerRepository, CampaignRepository campaignRepository, PledgeRepository pledgeRepository) {
    ...
    private Result<Pledge> validate(Pledge pledge) {
        ...
        if (campaign == null) {
            String msg = String.format("could not find campaign id %s", pledge.getCampaign().getCampaignId());
            result.addErrorMessage(msg);
        }

        Backer backer = backerRepository.findById(pledge.getBacker().getBackerId());
        if (backer == null) {
            String msg = String.format("could not find backer id %s", pledge.getBacker().getBackerId());
            result.addErrorMessage(msg);
        }
